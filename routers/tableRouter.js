...
tableRouter.post('/', (req, res) => {
    try {
        const { tableNumber, chairNumber, state } = req.body;
        TableModel.create(
            { tableNumber, chairNumber, state },
            (error, tableCreated) => {
...
tableRouter.put('/:tableId', async(req, res) =>{
    try {
        const { state, tableNumber, chairNumber} = req.body
        const updateInfo = { state, tableNumber, chairNumber }
        let tableFound = await TableModel.findById(req.params.tableId)
        if (!tableFound) res.status(404).send({ success: 0, message: " table not found" })
        else {
